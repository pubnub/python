...
if [[ $TRAVIS_PYTHON_VERSION == 3.7* ]]; then pip install -r requirements37-dev.txt; fi
if [[ $TRAVIS_PYTHON_VERSION == "nightly" ]]; then pip install -r requirements37-dev.txt; fi
