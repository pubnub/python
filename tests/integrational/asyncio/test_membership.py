...
def test_update_space_memberships(event_loop):
    ...
    envelope = yield from pn.update_space_memberships().user_id('charlie').limit(10).count(True)\
...
def test_update_members(event_loop):
    ...
    envelope = yield from pn.update_members().space_id('main').limit(10).count(True).include('custom')\
        .data(data).future()
