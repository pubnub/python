...
def test_update_space_memberships():
    ...
    envelope = pn.update_space_memberships().user_id('charlie').limit(10).count(True)\
        .include('custom').data(data).sync()
...
def test_update_members():
    ...
    envelope = pn.update_members().space_id('main').limit(10).count(True).include('custom').data(data).sync()
