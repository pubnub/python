...
class TestMessageActions(unittest.TestCase):
    ...
    def setUp(self):
        self.pubnub = PubNub(pnconf_env_copy())
        ...
        if TestMessageActions.message_timetoken is None:
            ...
            result = self.pubnub.publish().channel(TestMessageActions.channel).message(message_content).sync()
    ...
    def test_01_add_reactions(self):
        # Add first reaction
        add_result_1 = self.pubnub.add_message_action() \
        ...
        # Add second reaction
        add_result_2 = self.pubnub.add_message_action() \
            .channel(self.channel) \
            .message_action(PNMessageAction().create(
    ...
    def test_02_get_added_reactions(self):
        ...
        # Get all reactions
        get_reactions_result = self.pubnub.get_message_actions() \
            .channel(self.channel) \
            .sync()
        ...
        get_reactions_limited_result = self.pubnub.get_message_actions() \
            .channel(self.channel) \
            .limit('1') \
            .sync()
    ...
    def test_03_get_message_history_with_reactions(self):
        ...
        fetch_result = self.pubnub.fetch_messages() \
    ...
    def test_04_remove_reactions(self):
        # Ensure reactions were added by previous tests
        self.assertIsNotNone(TestMessageActions.action_timetoken_1, "Dependency: action_timetoken_1 not set")
        self.assertIsNotNone(TestMessageActions.action_timetoken_2, "Dependency: action_timetoken_2 not set")
        ...
        remove_result_1 = self.pubnub.remove_message_action() \
            .channel(self.channel) \
        ...
        remove_result_2 = self.pubnub.remove_message_action() \
            .channel(self.channel) \
            .message_timetoken(TestMessageActions.message_timetoken) \
        ...
        get_reactions_after_removal_result = self.pubnub.get_message_actions() \
            .channel(self.channel) \
            .sync()
    ...
    def test_05_remove_all_reactions(self):
        envelope = self.pubnub.get_message_actions() \
            .channel(self.channel) \
            .limit("100") \
            .sync()

        for action in envelope.result.actions:
            remove_result = self.pubnub.remove_message_action() \
                .channel(self.channel) \
