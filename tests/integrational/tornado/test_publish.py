...
class TestPubNubAsyncPublish(AsyncTestCase):
    ...
    def assert_success_publish_get(self, msg):
        self.assert_success(
            PubNubTornado(pnconf).publish().channel("my_channel").message(msg))

    def assert_success_publish_post(self, msg):
        self.assert_success(
            PubNubTornado(pnconf).publish().channel("my_channel").message(msg).use_post(True))

    def assert_success_publish_get_encrypted(self, msg):
        self.assert_success(
            PubNubTornado(pnconf_enc).publish().channel("my_channel").message(msg))

    def assert_success_publish_post_encrypted(self, msg):
        self.assert_success(
            PubNubTornado(pnconf_enc).publish().channel("my_channel").message(msg).use_post(True))
